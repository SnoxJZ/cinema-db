...
exports.getFavorites = async (req, res) => {
  ...
  const movieIds = user.favorites.map((id) => id.toString());
  ...
  const favorites = user.favorites
    .map((id) => {
      const movie = movieMap[id.toString()];
      ...
      return {
        ...
        status: movie.status,
        reviews: {
          ratingAvg: movie.reviewStats?.ratingAvg,
          reviewCount: movie.reviewStats?.reviewCount,
        },
      };
    })
    .filter(Boolean);
...
exports.getUserById = async (req, res) => {
  ...
  const user = await User.findById(userId).select(
    "name avatar isVerified role isBlocked blockedUntil favorites"
  );
  if (!user) return sendError(res, "User not found", 404);
...
exports.batchGetUsers = async (req, res) => {
  const { ids } = req.body;
...
exports.unblockAuto = async (req, res) => {
  const { userId } = req.params;
